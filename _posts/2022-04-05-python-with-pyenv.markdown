...
Но почему все-таки просто не использовать системный интерпретатор? Как следует из названия - системный интерпретатор - идет в пакете с системой по умолчанию (Linux и macOS). И обычно это как раз [старая версия python](https://www.macrumors.com/2022/01/28/apple-removing-python-2-in-macos-12-3/), на которой работают разные системные утилиты:
...
После установки добавляем в профиль оболочки код инициализации https://github.com/pyenv/pyenv#basic-github-checkout :
```bash
